...
glm::vec2 cart2pol(const glm::vec2 &cart) {
	return glm::vec2(hypot(cart.y,cart.x), atan2(cart.y,cart.x));
}

glm::vec2 pol2cart(const glm::vec2 &pol) {
	return glm::vec2(pol[0]*cos(pol[1]), pol[0]*sin(pol[1]));
}

glm::vec3 cart2cyl(const glm::vec3 &cart) {
	return glm::vec3(hypot(cart.y,cart.x),atan2(cart.y,cart.x),cart.z);
}

glm::vec3 cyl2cart(const glm::vec3 &cyl) {
	return glm::vec3(cyl[0]*cos(cyl[1]), cyl[0]*sin(cyl[1]),cyl.z);
}

glm::vec3 cart2sph(const glm::vec3 &cart) {
	return glm::vec3(sqrt(cart.x*cart.x+cart.y*cart.y+cart.z*cart.z),atan2(cart.y,cart.x),atan2(hypot(cart.x,cart.y),cart.z));
}

glm::vec3 sph2cart(const glm::vec3 &sph) {
	return glm::vec3(sph[0]*sin(sph[2])*cos(sph[1]), sph[0]*sin(sph[2])*sin(sph[1]),sph[0]*cos(sph[2]));
}
void draw_prism(unsigned sides, double radius, double height){
	...
	for(unsigned s=0;s<sides;++s){
		...
		auto t=glm::vec2(0.875f+0.125*sin(phi),0.75f-0.25f*cos(phi));
		glTexCoord2fv(&t[0]);
...
glEnd();
	...
	glNormal3fv(&top_normal[0]);
		for(unsigned s=0;s<sides;++s){
			...
			auto vertex_cart=cyl2cart(vertex_cyl);
			auto t=glm::vec2(0.875f+0.125*sin(phi),0.25f+0.25f*cos(phi));
				glTexCoord2fv(&t[0]);
			glVertex3fv(&vertex_cart[0]);
		}
	glEnd();
