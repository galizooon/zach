...
Model::Model(VerticesTypePtr vertices, FacesTypePtr faces, NormalsTypePtr normals,
		TexCoordsTypePtr tex_coords) :_vertices(vertices), _faces(faces), _normals(normals), _texcoords(tex_coords)
{
	assert(vertices!= nullptr && "NOT BE NULL");
		assert(normals!= nullptr && "NOT BE NULL");
		assert(faces!= nullptr && "NOT BE NULL");
		assert(tex_coords!= nullptr && "NOT BE NULL");
}
...
Model::Model(const char *filename) {
	...
	for (unsigned i = 0; i < vertex_count; ++i) {
		read_model_line(file, buffer);
		std::stringstream(buffer) >> vertices[i].x >> vertices[i].y
				>> vertices[i].z;
	}
	read_model_line(file, buffer);
...
//Описания нормалей
	...
	for (unsigned face = 0; face < face_count; ++face) {
		read_model_line(file, buffer);
		std::stringstream(buffer) >> normals[face].x >> normals[face].y
				>> normals[face].z;
	}
